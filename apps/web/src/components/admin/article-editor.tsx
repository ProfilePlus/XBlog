        ...
        <div className="admin-editor-stage">
          <section className="admin-card admin-section-card admin-editor-primary-card">
            <div className="admin-section-head">
              <div>
                <p className="admin-kicker">Drafting Studio</p>
                <h2>基础信息</h2>
              </div>
            </div>
            <div className="admin-form">
              <label>
                标题
                <input style={{ fontSize: '2rem', fontWeight: 700, fontFamily: 'var(--font-serif)' }} value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
              </label>
              <label>
                Slug
          ...
          <section className="admin-card admin-section-card admin-editor-primary-card" style={{ marginTop: '2rem' }}>
            <div className="admin-section-head">
              <div>
                <p className="admin-kicker">Content Composition</p>
                <h2>正文内容</h2>
              </div>
            </div>
              ...
              {form.blocks.map((block, index) => (
                <div key={block.id} className="admin-block-item">
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
                     <span className="admin-chip">{block.type}</span>
                     <button className="admin-ghost-button" style={{ padding: '0.2rem 0.6rem', fontSize: '0.6rem' }} onClick={() => removeBlock(index)}>REMOVE</button>
                  </div>
                  {block.type === 'paragraph' && (
                    <textarea 
                      value={block.text} 
                      onChange={(e) => updateBlock(index, { ...block, text: e.target.value })}
                      style={{ border: 'none', background: 'transparent', padding: 0, minHeight: '100px' }}
                      placeholder="开始在这里书写..."
                    />
                  )}
                  {block.type === 'heading' && (
                    <input 
                      value={block.text} 
                      onChange={(e) => updateBlock(index, { ...block, text: e.target.value })}
                      style={{ border: 'none', background: 'transparent', padding: 0, fontSize: '1.5rem', fontWeight: 600 }}
                      placeholder="章节标题..."
                    />
                  )}
                </div>
              ))}
              <div className="admin-block-actions" style={{ display: 'flex', gap: '1rem', marginTop: '2rem' }}>
                <button className="admin-ghost-button" onClick={() => addBlock("paragraph")}>+ 段落</button>
                <button className="admin-ghost-button" onClick={() => addBlock("heading")}>+ 标题</button>
              </div>
            </div>
          </section>
        </div>
